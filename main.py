...
def myprocessing(message):
    try:
        ...
        distance = geodesic(shake_location, current_location).kilometers
        ...
        if shake_info['unid'] != tmp_info['unid']:
            ...
            if 'create' in shake_info['action']:
                ...
                # Alarm if distance < 400
                if distance <= 400:
                    for i in range(7):
                        send_shake_message('‼️‼️‼️\n' + msg.format(**shake_info) + ' DISTANCE: ' + str(distance), False)
